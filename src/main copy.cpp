...
py::array_t<uint16_t> impute_image(const py::array_t<uint16_t> &image,
  ...
  auto srcImg =
      image.unchecked<2>();  // x must have ndim = 2; can be non-writeable
  ...
  py::array_t<float> calibrationImage2(
      {calibrationImage.shape(0), calibrationImage.shape(1)});
  auto clbImage =
      calibrationImage2.mutable_unchecked<2>();  // x must have ndim = 2; can be
                                                 // non-writeable
  auto srcClbImage =
      calibrationImage
          .unchecked<2>();  // x must have ndim = 2; can be non-writeable
    ...
    for (py::ssize_t j = 1; j < clbImage.shape(1) - 1; j++) {
      ...
      if (std::isless(calibrationValue, 0.0f)) {
        ...
        for (auto &indices : neighbouringIndices) {
          ...
          // check which pixels are 0 or greater around the query pixel in the
          // calibration image
